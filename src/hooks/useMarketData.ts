...
import { useSocket } from "../context/SocketContext"; // 👈 New Import
import type { ActiveAsset, CandleData } from "../types";

// ⚠️ STEP 5: Railway URL is now in SocketContext
...
export function useMarketData(asset: ActiveAsset, interval: string) {
    ...
    const fetchHistory = async () => {
      try {
        ...
        const isCrypto = (asset as any).type === "crypto" ||
          (asset.symbol.includes("/") && !asset.symbol.includes("USD"));
          ...
          "market-proxy",
          {
            body: {
              ...
              params: {
                ...
                interval: tdInterval,
                outputsize: "5000",
                exchange: isCrypto ? "binance" : undefined,
              },
            },
          },
        );
        ...
        if (isCrypto && (!data || !data.values)) {
          const { data: compData } = await supabase.functions.invoke(
            "market-proxy",
            {
              body: {
                endpoint: "time_series",
        ...
        if (validData && validData.values) {
          ...
          const formatted: CandleData[] = validData.values.reverse().map((
            d: any,
          ) => ({
  ...
  // 2. 📡 THE NEW FREE RECEIVER (Via Context)
  const { marketPrices } = useSocket();

  useEffect(() => {
    if (!asset.symbol) return;

    // Check if we have a price for this asset in the global context
    const price = marketPrices[asset.symbol];
    ...
    if (price) {
      ...
      setCandles((prev) => {
        ...
        // Only update if time is same, else wait for new candle logic (omitted for brevity)
        setLastCandleTime(last.time);
  ...
  }, [asset.symbol, marketPrices]); // 👈 Re-run when context updates this specific symbol
